...
const UpdateServiceArea = () => {
    ...
    useEffect(() => {
        const fetchServiceArea = async () => {
            try {
                const response = await fetch(`${process.env.REACT_APP_API_URL}service-areas/${id}/`);
    ...
    const handleSubmit = async (e) => {
        ...
        if (image && image.length > 0) {
            formData.append('image', image);  // New primary image
        }

        // Handle slider images
        if (images.length > 0) {
            // Append new images only if there are new images uploaded
                    ...
                    <CardBody>
                        <Form onSubmit={handleSubmit} encType="multipart/form-data">
                            <FormGroup>
                                ...
                                <Input id="areaName" name="areaName" placeholder="Enter Area Name" type="text" defaultValue={serviceAreaData.area} />
                            </FormGroup>
                            <FormGroup>
                            ...
                            {serviceAreaData &&serviceAreaData.images?
                            serviceAreaData.images.map((data)=>(
                                <img
                                src={data.image}
                                />
                            ))
                            ...
                            <FormGroup>
                                ...
                                <Editor
                                    ...
                                    initialValue={serviceAreaData.description || "<p>Write your content here</p>"}
                                    init={{
                                        height: 300,
                                        menubar: true,
                            ...
                            <FormGroup className="mt-3">
                                ...
                                <Editor
                                    ...
                                    initialValue={serviceAreaData.description2 || "<p>Write your content here</p>"}
                                    init={{
                                        height: 300,
                                        menubar: true,
                                        plugins: ['advlist autolink lists link image charmap print preview anchor', 'searchreplace visualblocks code fullscreen', 'insertdatetime media table paste code help wordcount'],
