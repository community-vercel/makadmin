...
const AddSocial= () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (serviceAreaData.icon==='' || image.length>0) {
        ...
        try {
            ...
            navigate(`/social`); // Redirect to the upobtained_dated service area page
        } catch (error) {
        ...
        <Row>
            <Col>
                <Card>
                  ...
                  {!serviceAreaData?.id?"Add Social":"Update Social"}      
                    </CardTitle>
                    <CardBody>
                        <Form onSubmit={handleSubmit} encType="multipart/form-data">
                            ...
                            <FormGroup>
                                ...
                                <Input id="platform" name="platform" placeholder="Enter name" type="text" defaultValue={serviceAreaData?.platform} />
                            </FormGroup>
                            <FormGroup>
                                <Label for="url">URL</Label>
                                <Input id="url" name="role" placeholder="Enter url" type="url" defaultValue={serviceAreaData?.url} />
                            </FormGroup>
                         
                            <FormGroup>
                            <FormGroup>
        <Label for="exampleFile">Upload Icon</Label>
        <Input id="exampleFile" name="file" type="file" onChange={handleImageChange} />
    </FormGroup>
    {serviceAreaData &&serviceAreaData.icon?
                                    <img
                                    src={serviceAreaData.icon}
                                    />
                                    :''
                                }
