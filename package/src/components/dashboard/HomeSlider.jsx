...
const HomeSliderTables = () => {
  const navigate = useNavigate();
  const [tableData, setTableData] = useState([]);
console.log(tableData)
  // Fetch Data from API
  useEffect(() => {
    const getdata=(()=>{ fetch(`${process.env.REACT_APP_API_URL}banners/`)
      .then(async response => {
  ...
  const handleEditClick = (id) => {
    ...
    navigate(`/update-homeslider/${id}`);
  };

  // Delete Function
  const handleDelete = (id) => {
    fetch(`${process.env.REACT_APP_API_URL}banners/${id}/`, {
      method: "DELETE",
    })
      .then((response) => {
        if (response.ok) {
  ...
  <div>
  <Button color="primary" onClick={() => navigate('/addslider')}>
              Add Slider
            </Button>
...
export default HomeSliderTables