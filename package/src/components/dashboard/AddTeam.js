...
const AddTeam= () => {
    ...
    useEffect(() => {
        const fetchServiceArea = async () => {
            try {
                const response = await fetch(`${process.env.REACT_APP_API_URL}team-members/${id}/`);
    ...
    const handleSubmit = async (e) => {
        ...
        if (serviceAreaData.photo==='' || image.length>0) {
        ...
        try {
            ...
            const response =!serviceAreaData?.id? await fetch(`${process.env.REACT_APP_API_URL}team-members/`, {
                method: 'POST',  // Use PUT method to upobtained_date
                body: formData,  // Attach formData
            }):await fetch(`${process.env.REACT_APP_API_URL}team-members/${id}/`, {
            ...
            navigate(`/team`); // Redirect to the upobtained_dated service area page
        } catch (error) {
        ...
        <Row>
            <Col>
                <Card>
                  ...
                  {!serviceAreaData?.id?"Add Team":"Update Certificate"}      
                    </CardTitle>
                    <CardBody>
                        <Form onSubmit={handleSubmit} encType="multipart/form-data">
                            ...
                            <FormGroup>
                                ...
                                <Input id="role" name="role" placeholder="Enter role" type="text" defaultValue={serviceAreaData?.role} />
                            </FormGroup>
                            <FormGroup>
                                <Label for="bio"> bio</Label>
                                <Input id="bio" name="bio" placeholder="Enter  bio" type="text" defaultValue={serviceAreaData?.bio} />
                            </FormGroup>
    ...
    {serviceAreaData &&serviceAreaData.photo?
                                    <img
                                    src={serviceAreaData.photo}
                                    />
                                    :''
                                }
